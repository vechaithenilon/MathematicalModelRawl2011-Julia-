...
Pkg.add("Plots")
Pkg.add("StatsPlots")
Pkg.add(Pkg.PackageSpec(name = "Gurobi", version = v"0.9.4"))
...
for i in (model_first:model_last)
    ...
    # Write result to data frames
    y_df = DataFrame(variable_type =[], index=[],value=[]);
    for (i,l) in set_IL
        val = y_star[(i,l)]
        push!(y_df, vcat("y",(i,l),val))
    end;


    r_df = DataFrame(variable_type =[],index=[],value=[]);
    for (k,i) in set_KN
        val = r_star[(k,i)]
        push!(r_df, vcat("r",(k,i),val))
    end;

    x_df = DataFrame(variable_type =[],index=[],value=[]);
    for (k,s,i,j) in set_KSA
        val = x_star[(k,s,i,j)]
        push!(x_df, vcat("x",(k,s,i,j),val))
    end;

    z_df = DataFrame(variable_type =[],index=[],value=[]);
    for (k,s,i) in set_KSN
        val = z_star[(k,s,i)]
        push!(z_df, vcat("z",(k,s,i),val))
    end;

    w_df = DataFrame(variable_type =[],index=[],value=[]);
    for (k,s,i) in set_KSN
        val = w_star[(k,s,i)]
        push!(w_df, vcat("w",(k,s,i),val))
    end;

    gamma_df = DataFrame(variable_type =[],index=[],value=[]);
    for s in set_S
        val = gamma_star[s]
        push!(gamma_df, vcat("gamma",s,val))
    end;

    # Merge all the data frames together
    result_df = DataFrame(variable_type=[], index=[], value=[]);
    result_df = DataFrames.append!(result_df,y_df);
    result_df = DataFrames.append!(result_df,r_df);
    result_df = DataFrames.append!(result_df,x_df);
    result_df = DataFrames.append!(result_df,z_df);
    result_df = DataFrames.append!(result_df,w_df);
    result_df = DataFrames.append!(result_df,gamma_df);

    # Export the merged data frame
    CSV.write("output_var$i.csv",result_df);

    # Export Objective value and Running Time (Add another column for model no)
    vars_no = length(y_star) + length(r_star) + length(x_star) + length(z_star) + length(w_star)
    constraint_no = length(fc) + length(ofc) + length(fpn) + length(ac) + length(adl) + 1 + length(dmrs) 
    stat_df = DataFrame(scenario=[], objective_value = [], modeling_time = [], solve_time = [], 
                        vars_no = [], y_no = [], r_no = [] , x_no = [], z_no =[] , w_no = [],
                        constraint_no = [], fc_no = [], ofc_no = [], fpn_no = [],ac_no = [], adl_no = [], rsd_no = [], dmrs_no = []
                        );
    DataFrames.push!(stat_df, vcat(i, JuMP.objective_value(m), round(TimeTrack_elapsed,digits = 5), round(JuMP.solve_time(m),digits = 5),
    vars_no,length(y_star), length(r_star), length(x_star), length(z_star), length(w_star),
    constraint_no,length(fc),length(ofc),length(fpn),length(ac),length(adl),1,length(dmrs)
    ));

    CSV.write("output_stat$i.csv",stat_df);

    println("Results successfully exported for scenario $i");
